...
// Taken from Texture64 and modified to work with Unity
namespace Texture64
{
   ...
   public class N64Graphics
   {
      ...
      public static System.Drawing.Color RGBA16Color(byte c0, byte c1)
      {
      ...
      public static System.Drawing.Color RGBA16Color(byte[] data, int pixOffset)
      {
      ...
      public static System.Drawing.Color RGBA32Color(byte[] data, int pixOffset)
      {
         int r, g, b, a;
         r = data[pixOffset];
         g = data[pixOffset + 1];
      ...
      public static System.Drawing.Color IA16Color(byte[] data, int pixOffset)
      {
         int i = data[pixOffset];
         int a = data[pixOffset + 1];
         return System.Drawing.Color.FromArgb(a, i, i, i);
      }

      public static System.Drawing.Color IA8Color(byte[] data, int pixOffset)
      {
         int i, a;
      ...
      public static System.Drawing.Color IA4Color(byte[] data, int pixOffset, int nibble)
      {
      ...
      public static System.Drawing.Color I8Color(byte[] data, int pixOffset, N64IMode mode = N64IMode.AlphaCopyIntensity)
      {
         int i = data[pixOffset];
         int a = i;
         switch (mode)
      ...
      public static System.Drawing.Color I4Color(byte[] data, int pixOffset, int nibble, N64IMode mode = N64IMode.AlphaCopyIntensity)
      {
         int shift = (1 - nibble) * 4;
      ...
      public static System.Drawing.Color CI8Color(byte[] data, byte[] palette, int pixOffset)
      {
         byte c0, c1;
      ...
      public static System.Drawing.Color CI4Color(byte[] data, byte[] palette, int pixOffset, int nibble)
      {
         byte c0, c1;
         int shift = (1 - nibble) * 4;
      ...
      public static System.Drawing.Color BPPColor(byte[] data, int pixOffset, int bit)
      {
         int i, a;
      ...
      public static System.Drawing.Color MakeColor(byte[] data, byte[] palette, int offset, int select, N64Codec codec, N64IMode mode)
      {
      ...
      static UnityEngine.Color32 InteropColor(System.Drawing.Color color)
      {
          return new UnityEngine.Color32(color.R, color.G, color.B, color.A);
      }
      static System.Drawing.Color InteropDrawingColor(UnityEngine.Color color)
      {
            return System.Drawing.Color.FromArgb(Mathf.RoundToInt(color.r * 255), Mathf.RoundToInt(color.g * 255), Mathf.RoundToInt(color.b * 255));
      }

      public static Texture2D RenderTexture(byte[] data, byte[] palette, int offset, int width, int height, N64Codec codec, N64IMode mode, bool point = false)
      {
         Texture2D g = new Texture2D(width, height, TextureFormat.RGBA32, false);
         // g.alphaIsTransparency = true;
         if (point)
         {
         ...
         for (int h = 0; h < height; h++)
         {
            for (int w = 0; w < width; w++)
            {
               ...
               {
                  UnityEngine.Color brush = InteropColor(MakeColor(data, palette, pixOffset, select, codec, mode));
                  g.SetPixel(w, h, brush);
               }
            }
         }
         g.Apply();
      ...
      public static void Convert(ref byte[] imageData, ref byte[] paletteData, N64Codec codec, Texture2D bm)
      {
         ...
         switch (codec)
         {
            case N64Codec.RGBA16:
               for (int y = 0; y < bm.height; y++)
               {
                  ...
                  {
                     System.Drawing.Color col = InteropDrawingColor(bm.GetPixel(x, y));
                     byte r, g, b;
                     r = SCALE_8_5(col.R);
                     g = SCALE_8_5(col.G);
                     b = SCALE_8_5(col.B);
