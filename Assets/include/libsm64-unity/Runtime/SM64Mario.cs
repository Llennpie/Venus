...
namespace LibSM64
{
    public class SM64Mario : MonoBehaviour
    {
        ...
        [SerializeField] bool lerpMario = true;

        GameObject marioRendererObject;
        [HideInInspector] public Mesh marioMesh;
        uint marioId;

        [SerializeField] PowerMeterHandler meter;
        ...
        public void contextUpdate()
        {
            ...
            for( int i = 0; i < lerpPositionBuffer.Length; ++i )
            {
                ...
                {
                    lerpPositionBuffer[i] = positionBuffers[buffIndex][i];
                    lerpNormalBuffer[i] = normalBuffers[buffIndex][i];
                }
            }
